...
letters = tuple(["C", "D", "E", "F", "G"])
c = [x.name for x in humans if x.name.startswith(letters)]
print(c)
...
e = [f"{x.name}-{str(x.age)}" for x in humans]
print(e)
...
f = [tuple([x.name, x.age]) for x in humans if x.age >= 27 and x.age <= 32]
print(f)
